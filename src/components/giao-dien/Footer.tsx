        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-16">
          ...
          <div>
            ...
            <ul className="space-y-4">
              {[
                { id: 'hero', label: language === 'vi' ? 'Trang chủ' : 'Home' },
                { id: 'menu', label: t.nav.menu },
                { id: 'careers', label: t.nav.careers || 'Careers' },
                { id: 'apply', label: t.nav.apply || 'Apply' },
                { id: 'partnership', label: t.nav.partner || 'Partnerships' },
                { id: 'about', label: t.nav.about },
                { id: 'contact', label: t.nav.contact },
              ].map(link => (
                <li key={link.id}>
                  <a href={`#${link.id}`} className="text-coffee-300 hover:text-amber-500 transition-colors">
                    {link.label}
                  </a>
                </li>
              ))}
