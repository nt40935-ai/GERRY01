...
import { Edit2, Trash2, Plus, X, ToggleLeft, ToggleRight } from 'lucide-react';
...
const ProductManager: React.FC<ProductManagerProps> = ({ products, categories, onAdd, onUpdate, onDelete, language }) => {
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    // Simple validation and type casting
  ...
  const toggleAvailability = (product: Product) => {
    onUpdate({ ...product, isAvailable: !product.isAvailable });
  };
        ...
        <table className="w-full text-left">
          <thead className="bg-coffee-50 text-coffee-900 font-semibold border-b border-gray-100">
            <tr>
              ...
              <th className="p-4">{t.admin_product.available}</th>
              ...
              <tr key={product.id} className="hover:bg-gray-50 transition-colors">
                <td className="p-4 flex items-center gap-3">
                  <div className="relative">
                    <img src={product.image} alt="" className={`w-10 h-10 rounded-lg object-cover bg-gray-100 ${!product.isAvailable ? 'grayscale opacity-50' : ''}`} />
                    {!product.isAvailable && (
                  ...
                  <span className={`font-medium ${!product.isAvailable ? 'text-gray-400' : 'text-coffee-800'}`}>{product.name}</span>
                </td>
                <td className="p-4 text-coffee-600">
                  <span className={`px-2 py-1 text-xs rounded-full ${product.category === ProductCategory.PROMOTION ? 'bg-red-100 text-red-800' : 'bg-coffee-100 text-coffee-800'}`}>
                    {(t.cats as any)[product.category] || product.category}
                  </span>
                </td>
                ...
                <td className="p-4">
                   ...
                   >
                     ...
                     {product.isAvailable ? 'On Sale' : 'Sold Out'}
                   </button>
                </td>
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
               ...
               <div>
                  ...
                  >
                    {categories.map(c => (
                      <option key={c.id} value={c.key}>{(t.cats as any)[c.key] || c.name}</option>
                    ))}
                  </select>
                </div>

              <div>
                <label className="block text-sm font-medium text-coffee-700 mb-1">{t.admin_product.available}</label>
                <div className="flex items-center gap-3">
                   <button
                     type="button"
                     onClick={() => setCurrentProduct({...currentProduct, isAvailable: true})}
                     className={`px-4 py-2 rounded-lg text-sm font-bold border transition-colors ${currentProduct.isAvailable ? 'bg-green-100 border-green-200 text-green-700' : 'bg-gray-50 border-gray-200 text-gray-500'}`}
                   >
                     Available
                   </button>
                   <button
                     type="button"
                     onClick={() => setCurrentProduct({...currentProduct, isAvailable: false})}
                     className={`px-4 py-2 rounded-lg text-sm font-bold border transition-colors ${!currentProduct.isAvailable ? 'bg-red-100 border-red-200 text-red-700' : 'bg-gray-50 border-gray-200 text-gray-500'}`}
                   >
                     Sold Out
                   </button>
                </div>
