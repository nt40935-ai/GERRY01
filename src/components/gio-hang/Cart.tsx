...
import { X, Trash2, Plus, Minus, ShoppingBag, ArrowRight, MessageSquare, Coffee, Layers } from 'lucide-react';
                  ...
                  <div className="flex-1 flex flex-col justify-between">
                    <div>
                      <div className="flex justify-between items-start">
                        ...
                        <p className="text-amber-600 font-medium whitespace-nowrap">{formatPrice(unitPrice, language)}</p>
                      </div>
                      
                      {/* Meta Info: Size, Toppings & Note */}
                      <div className="flex flex-wrap gap-2 mt-2">
                        {item.size && (
                          <span className="inline-flex items-center gap-1 text-[10px] font-bold bg-amber-50 text-amber-700 px-2 py-0.5 rounded border border-amber-100">
                             <Coffee className="w-3 h-3" /> Size {item.size}
                          </span>
                        )}
                        {item.toppings && item.toppings.length > 0 && (
                           <span className="inline-flex items-center gap-1 text-[10px] bg-purple-50 text-purple-700 px-2 py-0.5 rounded border border-purple-100" title={item.toppings.map(t => t.name).join(', ')}>
                             <Layers className="w-3 h-3" /> {item.toppings.length} Toppings
                           </span>
                        )}
                        {item.note && (
                           <span className="inline-flex items-center gap-1 text-[10px] bg-gray-50 text-gray-600 px-2 py-0.5 rounded border border-gray-100 max-w-full truncate">
                             <MessageSquare className="w-3 h-3" /> {item.note}
                           </span>
                        )}
                      </div>
                      {/* Detailed Toppings list */}
                      {item.toppings && item.toppings.length > 0 && (
                        <div className="mt-1 pl-1 border-l-2 border-purple-100">
